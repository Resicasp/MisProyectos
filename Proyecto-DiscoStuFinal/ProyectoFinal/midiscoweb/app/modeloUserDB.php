...
class ModeloUserDB {
	...
	private static $consulta_estado =	 "SELECT estado FROM Usuarios WHERE id=?";
	...
	private static $consulta_update3 = "UPDATE Usuarios set nombre=?, email=? WHERE id=?";
	private static $consulta_update4 = "UPDATE Usuarios set clave=?, nombre=?, email=? WHERE id=?";
	...
	public static function modeloObtenerEstado($user){
	    $estado=0;
	    $stmt = self::$dbh->prepare(self::$consulta_estado);
	    $stmt->bindValue(1,$user);
	    $stmt->execute();
	    if ($stmt->rowCount() > 0 ){
	        $stmt->setFetchMode(PDO::FETCH_ASSOC);
	...
	public static function Add($id,$nombre,$contra,$correo,$tipouser,$useracti){
		$dir="app/file";			
		if (!file_exists("app/file/".$id)) {
			mkdir("app/file/".$id, 0777, true);
		}
	...
	public static function UpdateUsuario ($user){
	    $stmt = self::$dbh->prepare(self::$consulta_user);
	    $stmt->bindValue(1,$user);
	    $stmt->execute();
	    if ($stmt->rowCount() > 0 ){
	    ...
	    include_once "plantilla/modificar2.php";
	}
	
	
	
	public static function UpdateUsuario2 ($id,$nombre,$contra,$correo){
	    $clave = password_hash($contra, PASSWORD_DEFAULT, ['cost' => 10]);
	    ...
	    if($contra==$campo2){
	        $stmt = self::$dbh->prepare(self::$consulta_update3);
	        $stmt->bindValue(1,$nombre);
	        $stmt->bindValue(2,$correo);
	        $stmt->bindValue(3,$id);
	        $stmt->execute();
	        ...
	        //echo $campo2."<br>".$clave."<br>".$contra;
	        $stmt = self::$dbh->prepare(self::$consulta_update4);
	        $stmt->bindValue(1,$clave);
	        $stmt->bindValue(2,$nombre);
	        $stmt->bindValue(3,$correo);
	        $stmt->bindValue(4,$id);
